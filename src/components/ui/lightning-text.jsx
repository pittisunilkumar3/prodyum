...
/**
 * LightningText
 * Renders the given text as a stroke outline, then sends lightning bolts +
 * sparks travelling across its pixels (canvas based, 21st.dev style).
 *
 * Adapted from the shadcn-style TSX snippet to JSX for this CRA project.
 *
 * Props:
 *  - text:        the string to render (default "21ST.DEV")
 *  - size:        font size in px (default scales with viewport)
 *  - color:       stroke colour of the text (default "#cd96fe")
 *  - interactive: when true, clicking adds a thunder burst. Set to false when
 *                 using the component as a background so it won't block clicks
 *                 on the content above it (default true).
...
export function LightningText({
  ...
  // Auto font size based on viewport if not provided
  const computeSize = () => {
    if (size) return size;
    const min = Math.min(window.innerWidth, window.innerHeight);
    return Math.max(48, Math.round(min / (textPropRef.current.length || 1) / 1.4));
  };
  ...
  useEffect(() => {
    ...
    // Initialize text with prop value
    textRef.current = new Text(
      { copy: textPropRef.current, size: computeSize(), color },
      w,
      h
    );
