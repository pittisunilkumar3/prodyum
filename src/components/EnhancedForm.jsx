        ...
        <div key={field.name} className={`relative ${field.fullWidth ? 'col-span-2' : ''}`}>
          <div className="relative">
            <textarea
              ...
              rows={5}
              className={`w-full px-6 py-4 pt-7 bg-white/[0.03] backdrop-blur-sm border-2 rounded-2xl text-white placeholder-transparent focus:outline-none transition-all duration-300 resize-none ${
                hasError ? 'border-prodyum-green-500' : isFocused ? 'border-prodyum-blue-500 shadow-[0_0_0_4px_rgba(30,136,229,0.12)]' : 'border-white/10 hover:border-white/25'
              }`}
          ...
          {hasError && <p className="text-prodyum-green-400 text-xs mt-1 ml-2">{hasError}</p>}
        </div>
      );
    }
        ...
        <div key={field.name} className={`relative ${field.fullWidth ? 'col-span-2' : ''}`}>
          <div className="relative">
            <select
              ...
              onFocus={() => setFocusedField(field.name)}
              onBlur={() => setFocusedField(null)}
              className={`w-full px-6 py-4 bg-white/[0.03] backdrop-blur-sm border-2 rounded-2xl focus:outline-none transition-all duration-300 appearance-none cursor-pointer ${
                hasError ? 'border-prodyum-green-500' : isFocused ? 'border-prodyum-blue-500 shadow-[0_0_0_4px_rgba(30,136,229,0.12)]' : 'border-white/10 hover:border-white/25'
      ...
      <div key={field.name} className={`relative ${field.fullWidth ? 'col-span-2' : ''}`}>
        <div className="relative">
          <input
            ...
            onFocus={() => setFocusedField(field.name)}
            onBlur={() => setFocusedField(null)}
            className={`w-full px-6 py-4 pt-7 bg-white/[0.03] backdrop-blur-sm border-2 rounded-2xl text-white placeholder-transparent focus:outline-none transition-all duration-300 ${
              hasError ? 'border-prodyum-green-500' : isFocused ? 'border-prodyum-blue-500 shadow-[0_0_0_4px_rgba(30,136,229,0.12)]' : 'border-white/10 hover:border-white/25'
            }`}
            placeholder={field.label}
          />
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <Button
        ...
        className="w-full bg-gradient-to-r from-prodyum-blue-500 to-prodyum-green-500 text-white hover:from-prodyum-blue-600 hover:to-prodyum-green-600 font-semibold py-6 transition-all duration-300 hover:shadow-2xl hover:shadow-prodyum-green-500/40 transform hover:scale-[1.02] group disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
      >
        {isSubmitting ? (
          <>
