  ...
  return (
    <div className="min-h-screen bg-black pt-32 pb-24">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Why ProDyum */}
        <div className="mb-20">
          <Card className="bg-gradient-to-br from-amber-500/10 via-transparent to-transparent border-amber-500/20 backdrop-blur-sm">
        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {productionServices.map((service) => (
              <Card
                key={service.id}
                className="bg-neutral-900 border-white/10 hover:border-amber-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-amber-500/10 group"
              >
                  ...
                  <ul className="space-y-3">
                    {service.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start text-sm text-gray-300">
                        <CheckCircle2 className="h-4 w-4 text-amber-500 mr-3 flex-shrink-0 mt-0.5" />
                        {feature}
                      </li>
                    ))}
                  </ul>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {postProductionServices.map((service) => (
              ...
              >
                <CardContent className="p-6">
                  <div className="mb-4">
                    <Volume2 className="h-8 w-8 text-amber-500 mb-3" />
                  ...
                  <ul className="space-y-2">
                    {service.features.map((feature, idx) => (
                      <li key={idx} className="flex items-center text-xs text-gray-500">
                        <CheckCircle2 className="h-3 w-3 text-amber-500 mr-2 flex-shrink-0" />
                        {feature}
                      </li>
                    ))}
                  </ul>
